...
export class WorkspaceService {
  ...
  async addVault(
    ...
    if (!config.site.duplicateNoteBehavior) {
      const vaults = DConfig.getConfig(config, "workspace.vaults") as DVault[];
  ...
  async shouldVaultsSync(
    ...
    if (_.isUndefined(workspaceVaultSyncConfig)) {
      if (await WorkspaceService.isWorkspaceVault(root)) {
        workspaceVaultSyncConfig =
          this.config.version === 3
            ? (this.config.workspace!.workspaceVaultSyncMode as DVaultSync)
            : this.config.workspaceVaultSync;
  ...
  async initialize(opts?: { onSyncVaultsProgress: any; onSyncVaultsEnd: any }) {
    ...
    const initializeRemoteVaults =
      this.config.version === 3
        ? this.config.workspace!.enableRemoteVaultInit
        : this.config.initializeRemoteVaults;
